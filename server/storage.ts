...
export interface IStorage {
  ...
  createJobImport(jobImportData: any): Promise<JobImport>;
  getJobImports(limit?: number): Promise<JobImport[]>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getCompanies(excludeUserId?: string): Promise<any[]> {
    try {
      ...
      // Add projects count manually
      const companiesWithCount = [];
      for (const company of companies) {
        const projectsCount = await prisma.project.count({
          where: { companyUserId: company.userId }
        });
  ...
  async getConversations(userId: string): Promise<Array<{
    ...
    try {
      ...
      const conversationUserIds = Array.from(new Set<string>());
      conversations.forEach(conv => {
        if (conv.senderId !== userId) conversationUserIds.push(conv.senderId);
        if (conv.receiverId !== userId) conversationUserIds.push(conv.receiverId);
      });
  ...
  // Job import operations
  async createJobImport(jobImportData: any): Promise<JobImport> {
    try {
  ...
  async getJobImports(limit: number = 50): Promise<JobImport[]> {
    try {
